...
sql for a flat table. think this is it - thank you Gastil. s
Still needs
- some testing- outer join on quantity?
...
, u.description
FROM   mob_sandbox.quantity q -- , mob_sandbox.unit u,  mob_sandbox.unitScope us 
LEFT OUTER JOIN mob_sandbox.unit u ON (u.quantityId = q.quantityId)
LEFT OUTER JOIN mob_sandbox.unit uu ON (uu.unitId = u.deprecatedTo) 
INNER JOIN mob_sandbox.unitScope us ON (us.unitId = u.unitId)
WHERE us.scopeId = 32 
-- and q.name LIKE '%oncentration%'
ORDER BY q.name
```
