...
- answer this: does the unit dictionary have to know about all possible quantities?
  - if unitDB quantities are expected to map to measurements, then yes.
  - if unitDB quantities are just a groouping for dimensions, then no. (and in this case, they should probably not be called quantities.)
